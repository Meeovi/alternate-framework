...
const globalForPrisma = globalThis as unknown as {
  __mframework_prisma: PrismaClientType | undefined
}
 ...
 * 
 * @example
 * import { initializePrisma } from '@mframework/core'
 ...
 * })
 */
export function initializePrisma(PrismaClientClass: any): PrismaClientType {
  ...
  if (!databaseUrl) {
    throw new Error(
      '[@mframework/core] Neither DATABASE_URL nor NUXT_DATABASE_URL environment variable is set. ' +
...
export function getPrisma(): PrismaClientType {
  if (!prismaInstance && !globalForPrisma.__mframework_prisma) {
    throw new Error(
      '[@mframework/core] Prisma not initialized. ' +
 ...
 * @returns The Prisma client instance
 */
export const useDB = async (_event?: any): Promise<PrismaClientType> => {
  return getPrisma()
}
