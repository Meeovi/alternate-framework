<template>
  <div>
    <form enctype="multipart/form-data"  v-for="segment in findManySegments" :key="segment.id" @submit.prevent="updateSegment(segment)">

      <nav class="navbar navbar-dark bg-dark">
        <div class="container-fluid">
          <a class="navbar-brand">
            <button type="reset" class="btn btn-warning" @click="deleteSegment(segment)">Delete</button></a>
          <a class="navbar-brand">
...
updateOneSegments(data: {name: $name, description: $description, website: $website, status: $status, apply_to: $apply_to} where: {id: $id}) {
  website
  name
  description
  status
  ...
  mutation deleteOneSegments($id: Int){
  deleteOneSegments(where: {id: $id}){
    website
  name
  description
  status
