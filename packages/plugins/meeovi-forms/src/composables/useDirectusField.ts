...
export function useDirectusField(field: any, modelValue: any, emit: any, formContext?: any) {
  ...
  const fieldLookup = computed(() => {
    ...
    if (['input', 'string'].includes(name)) return 'TextField'
    if (
      ['textarea', 'wysiwyg', 'rich-text', 'input-multiline', 'input-rich-text', 'input-rich-text-md'].includes(name)
    )
      return 'RichTextField'
    if (['select', 'select-dropdown', 'select-multiple'].includes(name)) return 'SelectField'
    if (name === 'datetime') return 'DateField'
    if (['file', 'files', 'file-image'].includes(name)) return 'FileUploadField'
