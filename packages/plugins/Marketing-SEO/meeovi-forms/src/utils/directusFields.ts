...
const FILE_INTERFACES = new Set(['file', 'file-image', 'file-video'])

/**
 * True for a single-file Directus field (a directus_files m2o — interface
 * file/file-image). The form uploads these to Pixanomy and stores the
 * public link in the sibling `<field>_url` string field instead, since a
 * URL can't go into the uuid relation itself.
 */
export function isFileField(field: DirectusField | null | undefined): boolean {
  if (!field) return false
  const iface = String((field.meta as any)?.interface ?? '').toLowerCase()
  const special = (field.meta as any)?.special
  return FILE_INTERFACES.has(iface) || (Array.isArray(special) && special.includes('file'))
}

/** The string field that holds a file field's Pixanomy URL. */
export const fileUrlFieldName = (field: string) => `${field}_url`
