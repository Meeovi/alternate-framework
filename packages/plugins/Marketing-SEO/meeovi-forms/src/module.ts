...
export interface MeeoviFormsModuleOptions {
  ...
  apis: ApiFormConfig[]
  /**
   * multipart endpoint file fields are uploaded to before submit; must
   * return `{ assets: [{ url }] }`. Defaults to Meeovi's /api/assets/upload
   * (Pixanomy).
   */
...
export default defineNuxtModule<MeeoviFormsModuleOptions>({
  ...
  setup (options: MeeoviFormsModuleOptions, nuxt: Nuxt) {
    ...
    nuxt.options.runtimeConfig.public.meeoviForms = {
      ...
      uploadEndpoint: options.uploadEndpoint || '/api/assets/upload',
      apis
    }
