...
<script setup lang="ts">
  ...
  const COLLECTION = (useRuntimeConfig().public.listTypeKanban as any)?.collection || 'list_items'
  ...
  function init(a: any) {
    ...
    a.on('move-card', async ({ id, card }: any) => {
      // Move = column change; persist status and sort order. The sort
      // column is `position` (what load() sorts by) — this used to write
      // `sort`, which list_items doesn't have, so order never persisted.
      await updateItem(COLLECTION, id, {
        status: card.column ?? 'todo',
        position: card.order ?? 0,
      })
      await load()
    })
