// ---------------------------------------------------------------------------
// SvelteKit integration for alternate-search
// ---------------------------------------------------------------------------
//
// Create a server route at:
//   src/routes/api/search/[index]/+server.ts
//
// Usage:
//   import { search } from "$lib/server/search";
//   import { toSvelteKitHandler } from "alternate-search/integrations/svelte";
//
//   const { GET, POST, DELETE, OPTIONS } = toSvelteKitHandler(search);
...
// Import RequestHandler type from @sveltejs/kit.
// It is a devDependency / peerDependency so always available in a SvelteKit
// project without being bundled into alternate-search itself.
...
export type SearchSvelteHandlerOptions = {
  /**
...
/**
 * Create SvelteKit `RequestHandler` exports for alternate-search.
 * Returns `{ GET, POST, DELETE, OPTIONS }` ready to be re-exported from a
 * `+server.ts` file.
 */
export function toSvelteKitHandler(
  search: SearchInstance,
  options: SearchSvelteHandlerOptions = {},
): { GET: RequestHandler; POST: RequestHandler; DELETE: RequestHandler; OPTIONS: RequestHandler } {
  // ── GET /:index ────────────────────────────────────────────────────────────
  const GET: RequestHandler = async ({ request, params, url }) => {
  ...
  // ── POST /:index ───────────────────────────────────────────────────────────
  const POST: RequestHandler = async ({ request, params, url }) => {
    const indexName = params.index ?? "";
    if (!indexName) return jsonError({ error: "Index name is required" }, 400);
  ...
  const DELETE: RequestHandler = async ({ request, params, url }) => {
    const indexName = params.index ?? "";
    if (!indexName) return jsonError({ error: "Index name is required" }, 400);
  ...
  const OPTIONS: RequestHandler = ({ request }) => {
