// ---------------------------------------------------------------------------
// TanStack Start (React) integration for alternate-search
// ---------------------------------------------------------------------------
//
...
//   import { toTanStackHandler } from "alternate-search/integrations/tanstack-start";
//
//   const { GET, POST, DELETE, OPTIONS } = toTanStackHandler(search);
//
...
export type SearchTanStackHandlerOptions = {
  /**
...
/** Event shape passed by TanStack Start to API route handlers. */
type TanStackRouteEvent = {
  request: Request;
  params: Record<string, string>;
};

/**
 * Create TanStack Start API route handlers for alternate-search.
 * Returns `{ GET, POST, DELETE, OPTIONS }` for use with `createAPIFileRoute`.
 */
export function toTanStackHandler(
  search: SearchInstance,
  options: SearchTanStackHandlerOptions = {},
): {
  GET: (event: TanStackRouteEvent) => Promise<Response>;
  POST: (event: TanStackRouteEvent) => Promise<Response>;
  DELETE: (event: TanStackRouteEvent) => Promise<Response>;
  OPTIONS: (event: TanStackRouteEvent) => Promise<Response>;
} {
  // ── GET /:index ────────────────────────────────────────────────────────────
  async function GET({ request, params }: TanStackRouteEvent): Promise<Response> {
    // TanStack Start uses $index as the param name in the route path
    const indexName = params.index ?? params["$index"] ?? "";
  ...
  // ── POST /:index ───────────────────────────────────────────────────────────
  async function POST({ request, params }: TanStackRouteEvent): Promise<Response> {
    const indexName = params.index ?? params["$index"] ?? "";
    if (!indexName) return jsonError({ error: "Index name is required" }, 400);
  ...
  async function DELETE({ request, params }: TanStackRouteEvent): Promise<Response> {
    const indexName = params.index ?? params["$index"] ?? "";
    if (!indexName) return jsonError({ error: "Index name is required" }, 400);
  ...
  async function OPTIONS({ request }: TanStackRouteEvent): Promise<Response> {
