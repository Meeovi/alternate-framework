export type FieldType = "string" | "number" | "boolean" | "date" | "string[]" | "number[]" | "geo" | "vector";
...
type InferFieldValue<T extends FieldDefinition> = T["type"] extends "number" ? number : T["type"] extends "boolean" ? boolean : T["type"] extends "date" ? string | Date : T["type"] extends "string[]" ? string[] : T["type"] extends "number[]" ? number[] : T["type"] extends "geo" ? GeoPoint : T["type"] extends "vector" ? number[] : string;
type InferRequiredFieldMap<T extends Record<string, FieldDefinition>> = {
    [K in keyof T as T[K]["stored"] extends false ? never : T[K]["optional"] extends true ? never : K]: InferFieldValue<T[K]>;
};
type InferOptionalFieldMap<T extends Record<string, FieldDefinition>> = {
    [K in keyof T as T[K]["stored"] extends false ? never : T[K]["optional"] extends true ? K : never]?: InferFieldValue<T[K]>;
};
type InferFieldMapDocument<T extends Record<string, FieldDefinition>> = InferRequiredFieldMap<T> & InferOptionalFieldMap<T>;
type InferFieldsArrayDocument<T extends readonly string[]> = {
    [K in T[number]]?: string;
};
export type InferSearchDocument<TSchema extends IndexSchema> = SearchDocument & (TSchema["fieldMap"] extends Record<string, FieldDefinition> ? InferFieldMapDocument<TSchema["fieldMap"]> : TSchema["fields"] extends readonly string[] ? InferFieldsArrayDocument<TSchema["fields"]> : Record<string, unknown>);
...
export type SearchMiddleware = (ctx: SearchMiddlewareContext, next: () => Promise<void>) => Promise<void> | void;
export type SearchLifecycleHook<TContext> = ((ctx: TContext) => Promise<void> | void) | Array<(ctx: TContext) => Promise<void> | void>;
...
export type SearchAnalyticsEventName = "search_performed" | "search_clicked" | "search_converted" | "search_error";
...
export type SearchAnalyticsEvent = SearchPerformedAnalyticsEvent | SearchClickAnalyticsEvent | SearchConversionAnalyticsEvent | SearchErrorAnalyticsEvent;
...
export type SearchMultiResult<TIndexes extends Record<string, IndexSchema>, TIndexName extends keyof TIndexes & string = keyof TIndexes & string> = {
    indexName: TIndexName;
    result: SearchResult<InferSearchDocument<TIndexes[TIndexName]>>;
};
export type SearchMultiStreamChunk<TIndexes extends Record<string, IndexSchema>, TIndexName extends keyof TIndexes & string = keyof TIndexes & string> = {
    indexName: TIndexName;
    result?: SearchResult<InferSearchDocument<TIndexes[TIndexName]>>;
    error?: Error;
};
export type SearchInstance<TIndexes extends Record<string, IndexSchema> = Record<string, IndexSchema>> = {
    ...
    index<TIndexName extends keyof TIndexes & string>(indexName: TIndexName, docs: InferSearchDocument<TIndexes[TIndexName]>[], options?: AdapterIndexOptions): Promise<void>;
    /** Execute the full search pipeline. */
    query<TIndexName extends keyof TIndexes & string>(indexName: TIndexName, query: SearchQuery, signal?: AbortSignal): Promise<SearchResult<InferSearchDocument<TIndexes[TIndexName]>>>;
    /** Execute multiple index queries concurrently. */
    queryMany<TIndexName extends keyof TIndexes & string>(requests: SearchRequest<TIndexes, TIndexName>[], signal?: AbortSignal): Promise<SearchMultiResult<TIndexes, TIndexName>[]>;
    /** Execute multiple index queries and yield each result as soon as it resolves. */
    queryManyStream<TIndexName extends keyof TIndexes & string>(requests: SearchRequest<TIndexes, TIndexName>[], signal?: AbortSignal): AsyncGenerator<SearchMultiStreamChunk<TIndexes, TIndexName>, void, void>;
    /** Execute federated multi-index search and fuse ranked results with RRF. */
    queryFederated<TIndexName extends keyof TIndexes & string>(requests: SearchRequest<TIndexes, TIndexName>[], options?: SearchFederatedOptions, signal?: AbortSignal): Promise<SearchFederatedResult<InferSearchDocument<TIndexes[TIndexName]>>>;
    ...
    stats(indexName: keyof TIndexes & string): Promise<{
        count: number;
        size?: number;
    }>;
};
