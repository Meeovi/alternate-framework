...
export const useCurrentOrganization = () => {
  return useState<FullOrganization | null>('organization', () => null)
}

export function useOrgs() {
  const { client } = useAuth()
  const organization = useCurrentOrganization()
  const activeOrganizationId = useCookie<string | null>('activeOrganizationId')
  ...
  async function createTeam(event: { data: CreateTeamSchema }, options: { showToast?: boolean } = {}) {
    const { showToast = true } = options
