...
export const localeData = currentLocales.map((l) => [l.code, l.files ? l.files : [l.file], l.name ?? l.code]);
...
async function readI18nFile(file) {
...
async function compare(baseEntries, file, data) {
    const baseEntriesKeys = Object.keys(baseEntries);
    const entries = await readI18nFile(file);
    const flatEntriesKeys = Object.keys(flatten(entries));
...
async function prepareTranslationStatus() {
    ...
    const entries = await readI18nFile(sourceLanguageLocale[1]);
    const flatEntries = flatten(entries);
    ...
    await writeFile(resolver.resolve('../mframework-translation-status.json'), JSON.stringify(translationStatus, null, 2), { encoding: 'utf-8' });
}
