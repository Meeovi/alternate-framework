...
export function formattedNumber(num, useFormatter = formatter) {
    return useFormatter.format(num);
}
export function useHumanReadableNumber() {
    const { n, locale } = _useI18n();
    const fn = (num) => {
        return n(num, num < 10000
            ? 'smallCounting'
            : num < 1000000
                ? 'kiloCounting'
                : 'millionCounting', locale.value);
    };
    return {
        ...
        forSR: (num) => unref(num) > 10000,
    };
}
export function useFormattedDateTime(value, options = { dateStyle: 'long', timeStyle: 'medium' }) {
    const { locale } = _useI18n();
...
export function useTimeAgoOptions(short = false) {
    const { d, t, n: fnf, locale } = _useI18n();
