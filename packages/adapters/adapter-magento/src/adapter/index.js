...
export function createMagentoCommerceAdapter(options) {
    const client = createMagentoClient(options);
    return {
        ...
        async addToCart(cartId, input) {
            return safeCall(async () => {
                const effectiveCartId = cartId ?? (await client.CreateCart()).createEmptyCart;
