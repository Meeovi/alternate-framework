...
function makeErr(err: unknown) {
  return { ok: false, error: (err as Error)?.message ?? String(err), data: null}
}
...
async function createOrganization(data: any, ownerId: string) {
  try {
    ...
    return { ok: true, data: org, error: null }
  } catch (e) {
    return makeErr(e)
  }
}
...
async function listOrganizationsForUser(userId: string) {
  try {
    if (!((prisma as any)?.organization) || !((prisma as any)?.organization_member)) return { ok: true, data: [], error: null }
    const orgs = await (prisma as any).organization.findMany({ where: { members: { some: { userId } } } })
    return { ok: true, data: orgs, error: null }
  } catch (e) {
    return makeErr(e)
  }
}
...
async function listMembers(organizationId: string) {
  try {
    if (!((prisma as any)?.organization_member)) return { ok: true, data: [], error: null }
    const members = await (prisma as any).organization_member.findMany({ where: { organizationId } })
    return { ok: true, data: members, error: null }
  } catch (e) {
    return makeErr(e)
  }
}
...
async function addMember(organizationId: string, userId: string, role = 'member') {
  try {
    ...
    return { ok: true, data: member, error: null }
  } catch (e) {
    return makeErr(e)
  }
}
...
async function createInvite(organizationId: string, inviterId: string, email: string) {
  try {
    ...
    return { ok: true, data: invite, error: null }
  } catch (e) {
    return makeErr(e)
  }
}
...
async function acceptInvite(code: string, userId: string) {
  try {
    ...
    return { ok: true, data: { organizationId: invite.organizationId }, error: null }
  } catch (e) {
    return makeErr(e)
  }
}
  ...
  plugins: [
    organization({
      ...
      updateOrganization: async (organizationId: string, updates: any) => {
        const res = await createOrganization(organizationId, updates)
