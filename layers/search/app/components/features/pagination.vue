...
const totalPages = computed<number>(() => {
    // Prefer an explicit totalPages provided by the storefront; otherwise compute from total and perPage
    if (storefront.totalPages !== undefined) return Number(unref(storefront.totalPages) || 1)
    const tot = Number(unref(storefront.total) || 0)
    const per = Number(unref(storefront.perPage) || 10)
    return Math.max(1, Math.ceil(tot / per))
})

const selectedPage = computed<number>(() => Number(unref(storefront.page) || 1))
...
const pages = computed(() => {
    ...
    const half = Math.floor(Number(maxVisiblePages.value) / 2)
    let start = Math.max(1, current - half)
    let end = Math.min(total, start + maxVisiblePages.value - 1)
