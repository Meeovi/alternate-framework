<template>
  <aside class="w-full md:max-w-94">
    <div class="flex justify-between mb-4">
      <h4 class="px-2 font-bold typography-headline-4">List settings</h4>
      <v-btn type="button" class="sm:hidden text-neutral-500" aria-label="Close filters panel">
        <SfIconClose />
      </v-btn>
    </div>
    <h5
        ...
        <SfAccordionItem v-model="opened[index]">
          ...
          <template v-if="type === 'checkbox'">
            <SfListItem v-for="{ id, value, label, counter } in details" :key="id" tag="label" size="sm"
              ...
              :class="['px-1.5 bg-transparent hover:bg-transparent', { 'font-medium': isItemActive(value) }]">
              <template #prefix>
                <SfCheckbox v-model="selectedFilters" wrapper-class="flex items-center" :disabled="counter === 0"
                  :value="value" />
              </template>
              <p>
    ...
    <div class="flex justify-between">
      ...
      <SfButton class="w-full" @click="showProducts">Show products</SfButton>
    </div>
  </aside>
</template>
...
<script lang="ts" setup>
  ...
  // Integrate with storefront composable so UI reacts to query and results
  ...
  const currentQuery = computed(() => storefront.query.value)
  const resultsTotal = computed(() => storefront.total?.value ?? 0)

  watch([selectedFilters, sortModel], async () => {
    // apply filter changes to storefront and search
    try {
      // map selected filters into facets or sort value
