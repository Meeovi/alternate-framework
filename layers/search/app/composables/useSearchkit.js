...
export function useSearchkit() {
    ...
    const client = nuxt.$searchClient;
    const indexName = nuxt.$searchIndexName || 'default';
    const helpers = nuxt.$searchHelpers || {};
    const query = ref('');
    const hits = ref([]);
    const total = ref(0);
    ...
    async function search() {
        ...
        try {
            ...
            if (Array.isArray(rawHits)) {
                // ES style hits.hits
                parsedHits = rawHits.map((h) => (h._source ? { ...h._source, _meta: h._id } : h));
            }
            else if (res.hits && res.hits.hits) {
                parsedHits = res.hits.hits.map((h) => (h._source ? { ...h._source, _meta: h._id } : h));
            }
            else if (res.hits) {
                parsedHits = res.hits;
            }
    ...
    async function autocomplete(input, limit = 6) {
        if (!client)
            return [];
        try {
            ...
            return Array.isArray(suggestionHits)
                ? suggestionHits.map((h) => (h._source ? h._source : h))
                : [];
        }
        catch (e) {
            return [];
    ...
    function setGeo(lat, lon, distanceKm = 50) {
        geo.value = { lat, lon, distanceKm };
        page.value = 1;
        return search();
    }
    function setSemantic(enabled) {
    ...
    const normalizedFacets = computed(() => {
        ...
        try {
            for (const k of Object.keys(facets.value || {})) {
                const v = facets.value[k];
                if (!v) {
                    out[k] = [];
                    continue;
                }
                if (Array.isArray(v)) {
                    out[k] = v.map((b) => ({ key: b.key ?? b.value ?? String(b), doc_count: b.doc_count ?? b.count ?? 0 }));
                }
                else if (v.buckets) {
                    out[k] = v.buckets.map((b) => ({ key: b.key ?? b.value, doc_count: b.doc_count ?? 0 }));
                }
                else if (v.terms) {
                    out[k] = v.terms.map((b) => ({ key: b.key ?? b.value, doc_count: b.doc_count ?? 0 }));
                }
                else {
                    // generic object — try to map entries
                    out[k] = Object.keys(v).map((kk) => ({ key: kk, doc_count: v[kk] }));
                }
            }
        }
        catch (e) {
