 ...
 *
 * Accepts a search-notification request from the frontend and delivers it
...
export default defineEventHandler(async (event) => {
  ...
  const searchCatalog = (client as any).search as Record<string, { send: (args: any) => Promise<any> }>
  const routeFn = searchCatalog[route]

  if (!routeFn || typeof routeFn.send !== 'function') {
    throw createError({
      statusCode: 400,
      statusMessage: `Unknown search notification route: ${route}`,
    })
  }
