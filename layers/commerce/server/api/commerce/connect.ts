...
let accountStatus: { chargesEnabled: any; id: any; payoutsEnabled: any; detailsSubmitted: any; } | null = null;
...
function setupEventListeners() {
  ...
  document.getElementById('create-product-form')!.addEventListener('submit', async (e) => {
    ...
    const productPrice = parseInt((document.getElementById('productPrice') as HTMLInputElement).value);
...
function renderAccountStatus() {
  ...
  accountStatusEl.innerHTML = `
    <div class="account-status">
      ...
      <button id="login-to-dashboard" class="button margin-bottom-10">Login to Dashboard</button>
  ...
  document.getElementById('login-to-dashboard')!.addEventListener('click', loginToDashboard);
...
async function createConnectAccount(email: any) {
  try {
...
async function createProduct(productName: any, productDescription: any, productPrice: number) {
  if (!accountId) return;
  ...
  try {
    ...
    (document.getElementById("productPrice") as HTMLInputElement).value = "1000";
  } catch (error) {
