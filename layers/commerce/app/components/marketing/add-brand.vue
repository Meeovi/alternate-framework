    ...
    <v-dialog v-model="dialog" :scrim="false" transition="dialog-bottom-transition">
      <template v-slot:activator="{ props }">
        <v-btn v-bind="props" class="rightAddBtn">
          <v-icon start icon="fas fa-plus"></v-icon>Create a Brand
        </v-btn>
      </template>
      <v-card class="b-1">
        <template>
        ...
        <template>
          ...
          <v-form @submit.prevent="submitForm">
            <DataFormElement v-for="field in brandFields" :key="field.field" :field="field" v-model="form[field.field]" />
...
const { form, formError, formSuccess, submitForm } = useDataForm('brands', brandFields, { clearOnSuccess: true, closeDialogRef: dialog })
</script>