...
type AddBtnStatus = 'add' | 'loading' | 'added';
...
export const useCart = () => {
  ...
  const fetchCart = async () => {
    // Cart is hydrated from localStorage in onMounted; extend here when a
    // server-backed cart endpoint is available.
  };

  const removeFromCart = (idOrKey: string | number) => {
    const key = String(idOrKey);
    const next = cart.value.filter(item => item.key !== key);
    setCart(next);
    void $fetch('/api/cart/update', {
      method: 'POST',
      body: { items: next.map(item => ({ key: item.key, quantity: 0 })) },
    });
  };

  const updateCartItem = (idOrKey: string | number, quantity: number) => {
    const key = String(idOrKey);
  ...
  const clearCart = () => {
    setCart([]);
    void $fetch('/api/cart/clear', { method: 'POST' });
  };
