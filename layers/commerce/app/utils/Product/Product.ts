...
export const getIndexedProduct = <T extends Partial<ProductItem | DomainProduct>>(
    product: T,
    itemSku?: string,
    ...
    const attributes = getIndexedAttributes(safeInitialAttributes);
...
export const getIndexedProducts = <T extends Partial<ProductItem | DomainProduct>>(
    products: T[],
...
export const getIndexedParameteredProducts = (
    ...
    .reduce((
        ...
        ...products,
        [id]: getIndexedProduct(product as any),
    }), {} as Record<string, IndexedWishlistProduct>);
...
export const showNewReviewPopup = (): void => {
    ...
    if (!isSignedIn() && !isGuestEnabled) {
        // Notification payload types differ between legacy and current implementations — cast to any
        const notif: any = (showNotification as any)(NotificationType.INFO, __('You must login or register to review products.'));
        dispatch(notif as any);
