...
export function useAuth() {
  ...
  const fetchSession = async () => {
    ...
    if (user.value) {
      if (payment == 'stripe') {
        ...
        const { data: customerState } = await (client as any).customer.state()
  ...
  if (import.meta.client) {
    client.$store.listen('$sessionSignal', async (signal: unknown) => {
      if (!signal)
        return
      await fetchSession()
    })
  ...
  return {
    ...
    forgetPassword: (client as any).forgetPassword,
