...
export default function useContentRequest() {
  ...
  async function readItems(collection: string, opts?: any) {
    const adapter = resolveAdapter()
    if (adapter && typeof adapter.readItems === 'function') return normalizeResult(await adapter.readItems(collection, normalizeOptions(opts)))
  ...
  async function readItem(collection: string, id: string, opts?: any) {
    const adapter = resolveAdapter()
    if (adapter && typeof adapter.readItem === 'function') return normalizeResult(await adapter.readItem(collection, id, normalizeOptions(opts)))
  ...
  async function createItem(collection: string, data: any) {
    const adapter = resolveAdapter()
    if (adapter && typeof adapter.createItem === 'function') return normalizeResult(await adapter.createItem(collection, normalizePayload(data)))
  ...
  async function updateItem(collection: string, idOrFilter: any, data: any) {
    const adapter = resolveAdapter()
    if (adapter && typeof adapter.updateItem === 'function') return normalizeResult(await adapter.updateItem(collection, idOrFilter, normalizePayload(data)))
