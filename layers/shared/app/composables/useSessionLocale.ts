...
export function useSessionLocale() {
  ...
  const persistLocaleForUser = (user = auth.user?.value, localeCode = locale?.value) => {
    ...
    if (!storageKey || !availableLocaleCodes.value.includes(localeCode)) return
  ...
  const restoreLocaleForUser = async (user = auth.user?.value) => {
    ...
    if (!storedLocale || storedLocale === locale?.value) return false

    if (typeof setLocale === 'function') await setLocale(storedLocale)
    return true
  }

  const setSessionLocale = async (localeCode: string) => {
    if (!availableLocaleCodes.value.includes(localeCode)) return false

    if (typeof setLocale === 'function') await setLocale(localeCode)
...
export default useSessionLocale
