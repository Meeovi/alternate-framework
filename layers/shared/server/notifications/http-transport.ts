...
import { z } from 'zod';
import { env, makeWelcomeInput } from './notify';
...
export const runHttpTransport = async (): Promise<void> => {
  const targetUrl = env.HTTP_TRANSPORT_URL ?? 'https://httpbin.org/post';

  const mail: Client<typeof catalog> = createClient({
    catalog,
    transportsByChannel: {
      email: httpTransport({
        url: targetUrl,
        apiKey: env.HTTP_TRANSPORT_API_KEY,
  ...
  const result = await mail.welcome.send({
    to: 'demo@example.com',
    input: makeWelcomeInput('Demo User'),
  });
