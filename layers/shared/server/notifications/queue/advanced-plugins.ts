import { createNotify, createClient, type Plugin } from '@betternotify/core';
...
import { z } from 'zod';
import { mockTransport, makeWelcomeInput } from '../notify';
...
export const runQueueAdvancedPlugins = async (): Promise<void> => {
  ...
  await mail.welcome.queue({ to: 'ada@example.com', input: makeWelcomeInput('Ada') });
  await mail.welcome.queue({ to: 'bob@example.com', input: makeWelcomeInput('Bob') });
