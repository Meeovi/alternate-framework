...
import { z } from 'zod';
import { env, mockTransport, makeWelcomeInput } from '../notify';
...
export const runQueueBullmq = async (): Promise<void> => {
  ...
  await mail.welcome.queue(
    { to: 'ada@example.com', input: makeWelcomeInput('Ada') },
    { bullmq: { delay: 1000, attempts: 3, backoff: { type: 'exponential', delay: 500 } } },
  );

  const batch = await mail.welcome.queueBatch(
    [
      { to: 'bob@example.com', input: makeWelcomeInput('Bob') },
      { to: 'carol@example.com', input: makeWelcomeInput('Carol') },
    ],
    { bullmq: { attempts: 3 } },
  );
