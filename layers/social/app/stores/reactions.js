...
export const useReactionsStore = defineStore('reactions', () => {
    ...
    let es = null;
    function key(contentId, contentType = 'posts') {
        return `${contentType}:${contentId}`;
    }
    function makeEntry(contentId, contentType = 'posts') {
    ...
    function getItem(contentId, contentType = 'posts') {
    ...
    async function defaultFetcher(contentId, contentType = 'posts') {
        const base = runtime.public?.apiBase || runtime.public?.directusUrl || '';
        try {
            if (!base)
                throw new Error('No API base configured for default reactions fetcher');
    ...
    async function defaultToggler(contentId, contentType = 'posts', currentlyLiked = false) {
        const base = runtime.public?.apiBase || runtime.public?.directusUrl || '';
        try {
            if (!base)
                throw new Error('No API base configured for default reactions toggler');
    ...
    function initAdapters(options) {
        if (options?.fetcher)
            fetcher = options.fetcher;
        if (options?.toggler)
            toggler = options.toggler;
        if (options?.realtimeUrl)
            initRealtime(options.realtimeUrl);
    }
    async function fetchReactions(contentId, contentType = 'posts', force = false) {
        const entry = getItem(contentId, contentType);
        if (!force && Date.now() - entry.lastFetched < 10_000)
            return entry;
        entry.loading = true;
        try {
            ...
            if ('userLikeId' in data)
                entry.userLikeId = data.userLikeId ?? null;
    ...
    async function toggleLike(contentId, contentType = 'posts') {
        const entry = getItem(contentId, contentType);
        if (entry.loading)
            return entry;
        entry.loading = true;
    ...
    function initRealtime(realtimeUrl) {
        // close existing
        if (es) {
            try {
                es.close();
            ...
            es.addEventListener('message', (ev) => {
                try {
                    ...
                    if (typeof payload.likeCount === 'number')
                        entry.likeCount = payload.likeCount;
                    if (typeof payload.isLiked === 'boolean')
                        entry.isLiked = payload.isLiked;
                }
                catch (e) {
