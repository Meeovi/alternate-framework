...
<template>
  <div flex flex-col w-full items-stretch gap-2 py3 dir="auto" class="poll-wrapper">
    <form v-if="!poll.voted && !poll.expired" flex="~ col gap3" accent-primary @click.stop="noop" @submit.prevent="vote">
      <div v-for="(option, index) of poll.options" :key="index" flex="~ gap2" items-center>
        <v-text-field name="choices" :value="index" :type="poll.multiple ? 'checkbox' : 'radio'" :label="option.title" cursor-pointer></v-text-field>
      </div>
    ...
    <template v-else>
      ...
      >
        ...
        <div class="bg-gray/40" rounded-l-sm rounded-r-lg h-5px w-full>
          <div bg-primary-active h-full min-w="1%" class="w-(--bar-width)" />
        </div>
      </div>
    </template>
