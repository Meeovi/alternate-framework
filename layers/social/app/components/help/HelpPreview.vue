...
<template>
  <div my-8 px-3 sm:px-8 md:max-w-200 flex="~ col gap-4" relative>
    <v-btn v-auto-focus type="v-btn" btn-action-icon absolute top--8 right-0 m1 :aria-label="$t('action.close')" @click="emit('close')">
    ...
    <p>
      {{ $t('help.desc_para4') }}
      <NuxtLink font-bold text-primary href="https://github.com/mframework-zone/mframework" target="_blank">
        {{ $t('help.desc_para5') }}
      </NuxtLink>
      {{ $t('help.desc_para6') }}
    </p>
    <NuxtLink hover:text-primary href="https://github.com/sponsors/mframework-zone" target="_blank">
      {{ $t('help.desc_para3') }}
    </NuxtLink>
    ...
    <p italic flex justify-center w-full>
      <NuxtLink href="https://github.com/sponsors/mframework-zone" target="_blank">
        <span text-xl font-script hover:text-primary transition duration-300>{{ $t('help.footer_team') }}</span>
      </NuxtLink>
    </p>

    <v-btn type="v-btn" btn-solid mxa @click="emit('close')">
      {{ $t('action.enter_app') }}
    </v-btn>
  </div>
</template>
