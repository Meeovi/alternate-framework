...
<script setup lang="ts">
    ...
    const { data: myVideoSpaces } = await useAsyncData<any[]>('myVideoSpaces', async () => {
        const resp = await $directus.request($readItems('spaces', { filter: { owner: { first_name: { _eq: currentFirstName.value }, last_name: { _eq: currentLastName.value } }, space_type: { space_types_id: { name: { _eq: 'Video' } } } }, fields: ['*', { '*': ['*'] }] }))
        return resp?.data || resp || []
    })

    const { data: videoSpaces } = await useAsyncData<any[]>('videoSpaces', async () => {
        const resp = await $directus.request($readItems('spaces', { filter: { space_type: { space_types_id: { name: { _eq: 'Video' } } } }, fields: ['*', { '*': ['*'] }] }))
        return resp?.data || resp || []
    })
</script>
