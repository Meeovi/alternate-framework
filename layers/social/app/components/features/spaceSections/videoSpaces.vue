...
<script setup lang="ts">
    ...
    const { data: myVideoSpaces } = await useAsyncData<any[]>('myVideoSpaces', async () => {
        const resp = await readItems('spaces', { filter: { owner: { first_name: { _eq: currentFirstName.value }, last_name: { _eq: currentLastName.value } }, space_type: { space_types_id: { name: { _eq: 'Video' } } } }, fields: ['*', { '*': ['*'] }] })
