...
<script setup lang="ts">
    ...
    const { data: myTextSpaces } = await useAsyncData<any[]>('myTextSpaces', async () => {
        ...
        const resp = await $directus.request($readItems('spaces', { filter: { user_created: { _eq: currentUser.value.id }, space_type: { space_types_id: { name: { _eq: 'Forum' } } } }, fields: ['*', { '*': ['*'] }] }))
        return resp?.data || resp || []
    })

    const { data: textSpaces } = await useAsyncData<any[]>('textSpaces', async () => {
        const resp = await $directus.request($readItems('spaces', { filter: { space_type: { space_types_id: { name: { _eq: 'Forum' } } } }, fields: ['*', { '*': ['*'] }] }))
        return resp?.data || resp || []
    })
</script>
