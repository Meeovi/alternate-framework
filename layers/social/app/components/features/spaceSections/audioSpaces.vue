...
<script setup lang="ts">
    ...
    const { data: myAudioSpaces } = await useAsyncData<any[]>('myAudioSpaces', async () => {
        const resp = await $sdk.content.readItems('spaces', { filter: { owner: { first_name: { _eq: currentFirstName.value }, last_name: { _eq: currentLastName.value } }, space_type: { space_types_id: { name: { _eq: 'Audio' } } } }, fields: ['*', { '*': ['*'] }] })
        return resp?.data || resp || []
    })

    const { data: audioSpaces } = await useAsyncData<any[]>('audioSpaces', async () => {
        const resp = await $sdk.content.readItems('spaces', { filter: { space_type: { space_types_id: { name: { _eq: 'Audio' } } } }, fields: ['*', { '*': ['*'] }] })
        return resp?.data || resp || []
    })
</script>
