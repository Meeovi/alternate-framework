...
<script setup lang="ts">
    ...
    const { data: myAudioSpaces } = await useAsyncData<any[]>('myAudioSpaces', async () => {
        ...
        const resp = await $directus.request($readItems('spaces', { filter: { user_created: { _eq: currentUser.value.id }, space_type: { space_types_id: { name: { _eq: 'Audio' } } } }, fields: ['*', { '*': ['*'] }] }))
        return resp?.data || resp || []
    })

    const { data: audioSpaces } = await useAsyncData<any[]>('audioSpaces', async () => {
        const resp = await $directus.request($readItems('spaces', { filter: { space_type: { space_types_id: { name: { _eq: 'Audio' } } } }, fields: ['*', { '*': ['*'] }] }))
        return resp?.data || resp || []
    })
</script>
