...
const buttonStyle = computed(() => {
  ...
  // If not following, use a v-btn style
  ...
  >
    ...
    <template v-else>
      <template v-if="relationship?.blocking">
        <span mframework-group-hover="hidden">{{ $t('account.blocking') }}</span>
        <span hidden mframework-group-hover="inline">{{ $t('account.unblock') }}</span>
      </template>
      <template v-if="relationship?.muting">
        <span mframework-group-hover="hidden">{{ $t('account.muting') }}</span>
        <span hidden mframework-group-hover="inline">{{ $t('account.unmute') }}</span>
      </template>
      <template v-else-if="relationship ? relationship.following : context === 'following'">
        <span mframework-group-hover="hidden">{{ relationship?.followedBy ? $t('account.mutuals') : $t('account.following') }}</span>
        <span hidden mframework-group-hover="inline">{{ $t('account.unfollow') }}</span>
      </template>
      <template v-else-if="relationship?.requested">
        <span mframework-group-hover="hidden">{{ $t('account.follow_requested') }}</span>
        <span hidden mframework-group-hover="inline">{{ $t('account.withdraw_follow_request') }}</span>
      </template>
      <template v-else-if="relationship ? relationship.followedBy : context === 'followedBy'">
        <span mframework-group-hover="hidden">{{ $t('account.follows_you') }}</span>
        <span hidden mframework-group-hover="inline">{{ account.locked ? $t('account.request_follow') : $t('account.follow_back') }}</span>
      </template>
      <template v-else>
