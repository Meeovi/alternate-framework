...
export async function createI18n(): Promise<LocalizedWebManifest> {
  ...
  const defaultShortcuts: ManifestOptions['shortcuts'] = [{
    ...
    name: nav?.notifications ?? 'Notifications',
  ...
  // Build a safe default manifest from the English i18n data — fallbacks used when
  // `pwa` or `pwa.webmanifest` is missing to avoid build-time exceptions.
  const defaultPwaEntry = pwa?.webmanifest?.[env] ?? {}
  const defaultManifest: Required<WebManifestEntry> = {
    name: defaultPwaEntry.name ?? (typeof nav?.home === 'string' ? nav.home : ''),
    short_name: defaultPwaEntry.short_name ?? (typeof nav?.home === 'string' ? nav.home : ''),
    description: defaultPwaEntry.description ?? '',
